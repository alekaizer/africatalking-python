...
class AfricasTalkingGateway:
    ...
    # Messaging methods
    def sendMessage(self, to_, message_, from_=None, bulkSMSMode_=1, enqueue_=0,
                    keyword_=None, linkId_=None, retryDurationInHours_=None,
                    authToken_=None):
    ...
    def fetchMessages(self, lastReceivedId_=0):
        ...
        self.getSmsUrl(), self.username, lastReceivedId_)
    ...
    def fetchPremiumSubscriptions(self, shortCode_, keyword_,
                                  lastReceivedId_=0):
    ...
    def paymentTransactionFindQuery(self,
        ...
        response = self.sendRequest(url + "?username=%s&transactionId=%s" % (
        self.username, transactionId_))
        if self.responseCode == self.HTTP_RESPONSE_OK:
            return json.loads(response)
        raise AfricasTalkingGatewayException(response)
    ...
    def mobilePaymentB2BRequest(self, productName_, providerData_,
                                currencyCode_, amount_, metadata_):
        ...
        if "destinationChannel" not in providerData_:
            raise AfricasTalkingGatewayException(
                "Missing field destinationChannel")
        ...
        if "destinationAccount" in providerData_:
            parameters['destinationAccount'] = providerData_[
                'destinationAccount']
